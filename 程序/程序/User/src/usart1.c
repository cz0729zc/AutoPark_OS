...
void USART1_IRQHandler(void) {
    static uint16_t rx_index = 0;  // 静态变量记录缓冲区写入位置

    // 处理接收中断
    if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
    {
        uint8_t data = USART_ReceiveData(USART1);  // 必须读取DR以清除标志

        // 缓冲区未满时存储数据
        if (rx_index < USART1_RXBUFF_SIZE)
        {
            Usart1_RxBuff[rx_index++] = data;
            Usart1_RxCounter++;
            
            // 检测结束符（例如回车或自定义协议尾）
            if (data == '\n' || data == '\r') 
            {
                Usart1_RxBuff[rx_index] = '\0';  // 添加字符串结束符
                Usart1_RxCompleted = 1;          // 设置完成标志
                rx_index = 0;                    // 重置索引
            }
        }
        else
